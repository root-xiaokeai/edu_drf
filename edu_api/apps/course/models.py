...
class Course(BaseModel):
    ...
    def lesson_list(self):
        "章节"
        lesson_list = CourseChapter.objects.filter(is_show=True, is_delete=False, course_id=self.id).all()
    ...
    @property
    def course(self):
        course = CourseLesson.objects.filter(is_show=True, is_delete=False, course_id=self.id).all()
        data_list = []
